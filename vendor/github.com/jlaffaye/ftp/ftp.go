...
// StartTLS Uses the AUTH TLS command to setup a TLS tunnel on the connection.
func (c *ServerConn) StartTLS(config *tls.Config) error {
	...
	conn := tls.Client(c.netConn, config)
...
func (c *ServerConn) cmdDataConnFrom(offset uint64, format string, args ...interface{}) (net.Conn, error) {
	...
	if c.tls {
		// TLS handshake on data connection if TLS is enabled
